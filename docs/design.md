...
- Ban abusive / known-bad IPs at the firewall for a Caddy reverse proxy, with minimal moving parts.
...
3. On enough hits within the window, the jail's `nftables` action drops the IP on tcp/80+443 for 90 days.
...
`report send-auto` flushes the queue to AbuseIPDB only when reporting is enabled **and** a key is present. It skips whitelisted IPs, dedupes per IP within `dedupe_window`, honors `daily_report_cap`, and requeues the remainder on a send failure or when the cap is hit. Comments are generic (never host/path/headers); categories are per-profile.
